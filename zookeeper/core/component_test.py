...
def test_component_post_configure():
    ...
    with pytest.raises(
        TypeError,
        match="The `__post_configure__` method of a @component class must take no arguments except `self` and `conf`",
    ):
    ...
    with pytest.raises(
        ...
        @component
        class C:
            def __post_configure__(self, conf, additional):
                pass
    ...
    class D:
        ...
        def __post_configure__(self, conf):
            self.c = self.a + self.b
            self.__configure_children__(conf)
