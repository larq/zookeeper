...
def task_sequence(cls):
    ...
    call_args = inspect.signature(cls.run).parameters
    if len(call_args) != 2 or len(call_args) == 2 and "self" not in call_args:
        raise TypeError(
            "A @task_sequence class must define a `run` method taking "
            f"`self` and `config` as arguments, but `{cls.__name__}.run` "
    ...
    def command(config, interactive):
        config = {k: v for k, v in config}
        task_instance = cls()
        remaining_conf = configure(
            task_instance, config, interactive=interactive, skip_subtasks=True
        )
        task_instance.run(remaining_conf)
