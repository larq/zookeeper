# Forked from https://github.com/tensorflow/tensorflow/blob/master/tensorflow/contrib/training/python/training/hparam.py
...
def _cast_to_type_if_compatible(name, param_type, value):
    ...
    if issubclass(param_type, (str, bytes)) and not isinstance(value, (str, bytes)):
    ...
    def to_json(self, indent=None, separators=None, sort_keys=False):
        ...
        def remove_callables(x):
            ...
            if isinstance(x, dict):
                return {k: remove_callables(v) for k, v in x.items() if not callable(v)}
